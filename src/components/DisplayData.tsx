...
import { AiOutlinePlus, AiOutlineArrowDown, AiOutlineArrowLeft, AiOutlineArrowRight } from 'react-icons/ai'
...
function DisplayData() {
    ...
    const handleModalData = async (e: React.FormEvent<HTMLFormElement>, id = tempId) => {
        e.preventDefault();
        ...
        if (hook.firstName != undefined && hook.lastName != undefined && hook.status != undefined && hook.role != undefined && hook.email != undefined) {
            const upperCaseFirstName = hook.firstName.value.charAt(0).toUpperCase() + hook.firstName.value.slice(1)
            const upperCaseLastName = hook.lastName.value.charAt(0).toUpperCase() + hook.lastName.value.slice(1)
            const upperCaseStatus = hook.status.value.charAt(0).toUpperCase() + hook.status.value.slice(1)
            const upperCaseRole = hook.role.value.charAt(0).toUpperCase() + hook.role.value.slice(1)
            ...
            if (id === '') {
                ...
                const uploadData: DataInterface = {
                    firstName: upperCaseFirstName ? upperCaseFirstName : (userData?.firstName ? userData.firstName : ''),
                    lastName: upperCaseLastName ? upperCaseLastName : (userData?.lastName ? userData.lastName : ''),
                    role: upperCaseRole ? upperCaseRole : (userData?.role ? userData.role : ''),
                    status: upperCaseStatus ? upperCaseStatus : (userData?.status ? userData?.status : ''),
                    email: hook.email.value ? hook.email.value : (userData?.email ? userData.email : ''),
    ...
    const handleEditUser = async (id: string) => {
        ...
        await fetch(`${url}/${customId}`).then(data => data.json()).then(tempData => setUserData(tempData));
        setShowModal(true);
        setTempId(id);
    }
            ...
            <div className="relative my-6 mx-auto w-[50vw]">
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                    ...
                    <div className="relative p-6 flex-auto">
                        <form onSubmit={(e) => handleModalData(e)} className="bg-gray-200 shadow-md rounded px-8 pt-6 pb-8 w-full">
                            <label className="block text-black text-sm font-bold mb-1">
                                First Name
                            </label>
                            <input name='firstName' className="shadow appearance-none border rounded w-full py-2 px-1 text-black" />
                            <label className="block text-black text-sm font-bold mb-1">
                                Last Name
                            </label>
                            <input name='lastName' className="shadow appearance-none border rounded w-full py-2 px-1 text-black" />
                            <label className="block text-black text-sm font-bold mb-1">
                                Email
                            </label>
                            <input name='email' className="shadow appearance-none border rounded w-full py-2 px-1 text-black" />
                            <label className="block text-black text-sm font-bold mb-1">
                                Role
                            </label>
                            <input name='role' className="shadow appearance-none border rounded w-full py-2 px-1 text-black" />
                            <label className="block text-black text-sm font-bold mb-1">
                                Status
                            </label>
                            <input name='status' className="shadow appearance-none border rounded w-full py-2 px-1 text-black" />
                    ...
                    <div className='flex flex-col justify-center items-center mt-[5%]'>
                        <div className="flex flex-col border md:w-[80vw] rounded-lg shadow-lg">
                            <div className="p-4 flex justify-between border-b">
                                <div className="flex flex-col">
                                    <div className="flex mb-3">
                                        ...
                                        <span className='rounded border text-green-400 text-sm p-1'>{clientData.length} users</span>
                                    </div>
                                    <p className='text-gray-500 text-sm sm:w-1/2 md:w-auto'>Manage your team members and their account permissions here.</p>
                                </div>
                                <div className="flex p-2">
                                    <DownloadButton data={clientData} />
                                    <button onClick={() => setShowModal(prev => !prev)} className="text-white bg-blue-400 border shadow-md flex justify-center items-center rounded-lg px-3">
                            ...
                            <div className="flex justify-between p-2 border-b">
                                <span onClick={() => handleSorting('firstName')} className="flex text-gray-400 text-sm items-center justify-center ml-8 cursor-pointer">
                                    Name &nbsp; <AiOutlineArrowDown fontSize={15} />
                                </span>
                                <div onClick={() => handleSorting('status')} className="flex p-2 w-1/2 justify-between">
                                    ...
                                    <span onClick={() => handleSorting('role')} className="flex text-gray-400 text-sm items-center justify-center mx-2 cursor-pointer">
                                        Role &nbsp; <AiOutlineArrowDown fontSize={15} />
                                    </span>
                                    <span onClick={() => handleSorting('login')} className="flex text-gray-400 text-sm items-center justify-center mx-2 cursor-pointer">
                            ...
                            </div>
                            {
                                currentPageData.map((user, idx) => {
                                    ...
                                    return (
                                        <div key={idx} style={tempStyle} className='flex justify-between border-b border-black text-sm p-2'>
                                            <div className='flex justify-center items-center'>
                                                <img className='w-12 h-12 rounded-full' src={`/images/avatar${user?.img}.png`} alt="" />
                                            ...
                                            <div className="flex w-1/2 -mr-2 items-center">
                                                ...
                                                <div className='text-gray-400 mr-2 text-sm w-1/5'>{user.role.charAt(0).toUpperCase() + user.role.slice(1)}</div>
                                                ...
                                                <div className="flex mx-2 text-sm justify-evenly md:w-1/6 lg:w-1/3 ml-10">
                                                    <RiDeleteBin6Line onClick={() => handleDeleteRequest(user.id)} fontSize={21} className="cursor-pointer" />
                                                    <VscEdit onClick={() => handleEditUser(user.id)} fontSize={21} className="cursor-pointer" />
                                                </div>
                                            </div>
                                        </div>
                                    )
                            ...
                            <ReactPaginate
                                ...
                                previousLabel={
                                    <div className='flex items-center justify-around text-sm mx-2'>
                                        <AiOutlineArrowLeft fontSize={15} className="" />&nbsp;Previous
                                    </div>
                                }
                                nextLabel={
                                    <div className='flex items-center'>
                                        <span>Next</span>&nbsp;<AiOutlineArrowRight fontSize={18} />
                                    </div>
                                }
                            />
                        </div>
